...
(function () {
  ...
  **/
  function focusableAreas(option = {'mode': 'visible'}) {
    ...
    return (option.mode === 'all') ? focusables.filter(isVisibleWithScroll) : focusables.filter(isVisible);
  }
  ...
  **/
  ...
  function isVisibleWithScroll(element) {
    const elementStyle = window.getComputedStyle(element, null);
    return (!element.parentElement) || (isVisibleStyleProperty(elementStyle) && (hitTest(element, elementStyle) || hitTestWithScroll(element)));
  }
  /**
  * isEntirelyVisible :
  ...
  **/
  ...
  function elementFromPointInWholeDocument(x,y) {
    ...
    /* Grab the element */
    let elm = document.elementFromPoint(newX,newY);
  ...
  **/
  function hitTestWithScroll(element) {
    ...
    const elementRectValue = getBoundingClientRect(element);
    let scrollX = window.pageXOffset;
    let scrollY = window.pageYOffset;

    const elementRect = {
      left: elementRectValue.left + scrollX,
      right: elementRectValue.right + scrollX,
      top: elementRectValue.top + scrollY,
      bottom: elementRectValue.bottom + scrollY,
    };
